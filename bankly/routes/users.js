 ...
 *
 */
//BUG: Lists too much information, more than mentioned in docstring, used map to format.
...
//BUG: requires admin when user auth should be enough 
// router.patch('/:username', authUser, requireLogin, requireAdmin, async (req, res, next) => {
router.patch('/:username', authUser, requireLogin, async (req, res, next) => {
  try {
    ...
    //BUG: no validation for fields, allowing update of username, password, is_admin
    const validate = ['first_name', 'last_name', 'phone', 'email'];
    for (let key in fields) {
      if (!validate.includes(key)) {
        throw new ExpressError(`Not allowed to change ${key}`, 401);
      };
    };
...
router.delete('/:username', authUser, requireAdmin, async (req, res, next) => {
  try {
    // BUG: User.delete doesn't use await
